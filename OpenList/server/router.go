...
func _fs(g *gin.RouterGroup) {
	...
	// Chunked upload (server-side chunked upload for large files / cloudflare tunnel)
	chunk := g.Group("/chunk", middlewares.FsUp)
	chunk.POST("/init", handles.ChunkUploadInit)
	chunk.POST("/upload", handles.ChunkUploadPart)
	chunk.POST("/complete", handles.ChunkUploadComplete)
	chunk.GET("/status", handles.ChunkUploadStatus)
	chunk.POST("/abort", handles.ChunkUploadAbort)
}
